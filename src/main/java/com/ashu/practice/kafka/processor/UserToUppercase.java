...
public class UserToUppercase {
    ...
    public void process(StreamsBuilder streamsBuilder) {
        ...
        log.info("Processing stream for uppercase");
        ...
        messageStream.mapValues(user -> {
            user.setName(user.getName().toUpperCase());
            user.setCity(user.getCity().toUpperCase());
            return user;
        }).to(Constants.TOPIC_USER_UPPER_CASE, Produced.with(keySpecificAvroSerde, valueSpecificAvroSerde));
    }
}
